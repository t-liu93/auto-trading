...
def prepare_smas(seis: SeisData) -> str:
...
def calculate_indicator(seis: SeisData) -> str:
    sma = calculate_sma(seis)
    return sma
...
def calculate_sma(seis: SeisData) -> str:
    ...
    if compare_crossing(new_indicators['sma_4'].iloc[0], new_indicators['sma_4'].iloc[1], new_indicators['sma_7'].iloc[0], new_indicators['sma_7'].iloc[1]) == CrossingType.UP:
        return 'slightly bull'
    elif compare_crossing(new_indicators['sma_4'].iloc[0], new_indicators['sma_4'].iloc[1], new_indicators['sma_7'].iloc[0], new_indicators['sma_7'].iloc[1]) == CrossingType.DOWN:
